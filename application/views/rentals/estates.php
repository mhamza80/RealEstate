...
<?php if(empty($rows)){?>
		...
		<div id="message-warning" class="message message-warning">
						...
						<table id="emp" border="2" class="jtable">
					        <thead><tr><th>رقم </th><th>نوع العقار</th><th>عدد الوحدات</th></tr>
					        	<?php foreach ($rows as $key => $list):
					        		$count = $this->type->count_estates($list['id']);
					      			$i++; ?>
					         </thead>
					             <tr>
					               <td><?php echo $i?></td>
						           <td><?php echo anchor("admin/rentals/directory/".$list['id'],$list['type']);?></td>
