...
<?php if ($this->session->flashdata('sucess')){?>
	...
	<div class="table">
			<div id="dialog-add-producer" title="إضافة نوع عقار  جديد">
				<p class="validateTips">جميع الحقول إحبارية.</p>
				<?php echo form_open_multipart('admin/types/add','id=estate' )?>
				<div class="form">
					<div class="fields">
						<div class="field">
							<div class="label">
								<label for="autocomplete">نوع العقار:</label>
							</div>
							<div class="input">
...
<?php if(empty($rows)){?>
		<button id="add-producer">إضافة</button> <br/>
		<div id="message-warning" class="message message-warning">
								...
								<div class="text">
									<span><?php echo "لا توجد أنواع عقارات مضافة"?></span>
								</div>
								<div class="dismiss">
							...
							if(is_array($rows)){$i = 0;
									echo '<button id="add-producer">إضافة</button> <br/>';?>
						<table id="emp" border="2" class="jtable">
					        <thead><tr><th>رقم </th><th>نوع العقار</th><th>عدد الوحدات</th><th>الإجراءات</th></tr>
					        	<?php foreach ($rows as $key => $list):
					             ...
					             <tr>
					               <td><?php echo $i?></td>
						           <td><?php echo anchor("admin/SubTypes/add/".$list['id'],$list['type']);?></td>
