...
<div class="box">
			...
			<?php if(empty($result)):?>
				<div id="message-warning" class="message message-warning">
							...
							<?php endif;			
								if(is_array($result)){$i = 0;?>								
								<table id="emp" border="2" class="jtable">
					       			 <thead><tr><th>رقم</th><th>القسم</th><th>المبلغ</th><th>التاريخ</th><th>الإيضاح</th></tr>
					        		<?php foreach ($result as $key => $list):$i++;?>
					        		 </thead>
					                <tr>
					               		 <td><?php echo $i?></td>
					          		  ...
					          		  <?php endforeach;?>
					            		<tfoot>
    										 <TR> <td colspan="6"> مجموع الإيردات: <?php echo number_format(round($count->amount),0,'.',",")." " ."ريال";?></td></tr>
										</tfoot>
								</table>
							<?php }
